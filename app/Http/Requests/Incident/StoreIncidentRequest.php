...
class StoreIncidentRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            // Check room_id phải thuộc về property_id
            'room_id' => [
                'nullable',
                'integer',
                'exists:rooms,id',
                function ($attribute, $value, $fail) {
                    if ($value) {
                        $exists = \App\Models\Room::where('id', $value)
                            ->where('property_id', $this->property_id)
                            ->exists();
                        if (!$exists) {
                            $fail('Phòng không thuộc khu nhà này.');
                        }
                    }
                }
            ],
            'reported_by_tenant_id' => ['nullable', 'integer', 'exists:tenants,id'],
            'title' => ['required', 'string', 'max:255'],
            'description' => ['nullable', 'string'],
            'category' => ['required', new Enum(IncidentCategory::class)],
            'priority' => ['required', new Enum(IncidentPriority::class)],

            // Validate mảng ảnh (ảnh before_repair)
